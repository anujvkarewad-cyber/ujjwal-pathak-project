// Minimal pure-TypeScript SHA-256 + HMAC implementation (no native crypto
// dependency), used to compute the X-Sync-Token header for /api/progress-sync.
// Mirrors the backend: hex(HMAC-SHA256(secret, studentId)).
...
export const hmacSha256 = (secret: string, message: string): string => {
  if (!secret) return '';
  const key = utf8(secret);
  const block = 64;
  const normalized = key.length > block
    ? sha256(key)
    : key.length === block ? key : new Uint8Array([...key, ...new Uint8Array(block - key.length)]);
  const inner = new Uint8Array(block + message.length);
  const outer = new Uint8Array(block + 32);
  for (let i = 0; i < block; i += 1) inner[i] = normalized[i] ^ 0x36;
  for (let i = 0; i < block; i += 1) outer[i] = normalized[i] ^ 0x5c;
  inner.set(utf8(message), block);
